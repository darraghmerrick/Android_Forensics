...
my $rw = `adb shell su -c "mount -o remount, rw /dev "`;
	...
	my $move = adb shell su -c "cp -R $_ /dev/google_extract"
	print " Extracting $_ ";
	$i++;	
					     }
                         } 
	...
	#my $remove = adb shell su -c "rmdir -r /dev/google_extract"
	
	}
	print " Thank you for using Android Google data Extract \n";
