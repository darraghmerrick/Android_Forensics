...
my $kill =`adb wait-for-device`;          # Ensure graceful start.
	...
	my $temp = `adb shell su -c "mkdir /dev/android_providers "`;
	...
	foreach (@data2)
	 {
   	 if ($_ =~ /^com.android.providers/) { # Examines mounted partitions for the presence of ext4 filesystem.
	my $move = `adb shell su -c "cp -R /data/data/$_ /dev/android_providers/"`;
	print " Extracting $_ \n ";
	$i++;	
                                             } 
	 }
        if ($i==0) {
	print " No com.Android.providers data was extracted :-(\n ";
	
	}
	if ($i>0)  {
	my $permissions = `adb shell su -c "chmod -R 755 /dev/android_providers"`;
	my $extract =`adb pull /dev/android_providers/`;
	my $remove = `adb shell su -c "rm -r /dev/android_providers"`;
